...
import { Lock, Calculator, Clock, CalendarDays, History, ChevronDown, ChevronUp, X, ArrowLeft, FileText } from "lucide-react"
                ...
                {historialFiltrado.map((c: any) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-gray-50 hover:bg-emerald-50 rounded-lg p-3 border border-gray-100 hover:border-emerald-200 cursor-pointer transition-colors group"
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-400">{format(new Date(c.dateFrom), "dd/MM")}</span>
                          <span className="text-xs text-gray-400">{format(new Date(c.dateFrom), "HH:mm")} → {format(new Date(c.dateTo), "HH:mm")}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="font-black text-emerald-600 text-sm">${c.total.toLocaleString()}</span>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setSelectedClosure(null)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="bg-emerald-50 border-b border-emerald-100 px-5 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {isGeneral
                    ? <Badge className="bg-blue-100 text-blue-800 border-blue-200">GENERAL · {displayName}</Badge>
                    : <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200">{displayName}</Badge>
                  }
                  <button onClick={() => setSelectedClosure(null)} className="text-gray-300 hover:text-gray-500 transition-colors ml-1">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              </div>
